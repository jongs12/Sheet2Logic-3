...
버전 3의 경우 BPM 변경 시 인게임에서 print로 변경된 위치를 알려주고,
버전 3.5의 경우 ## 이후 내용을 적으면 해당 내용이 적힌 위치를 print로 기록합니다.
...
제대로 입력했다면 버전 3에서는 각 트랙별로 반복 횟수, 음정 조절, 속도 조절, 시작 오프셋, 반복 딜레이를 입력받습니다.
...
버전 3.5에서는 프로그램 실행 시 묻지 않고, 파일 자체에 기록하여 조절합니다.
프로그램에서 인식하는 문자열은 "!반복", "!피치", "!속도", "!악기"가 있습니다.
해당 문자열 입력 후 한 칸을 띄우고 값을 입력하면 됩니다.

노트블록에서 지원하는 악기에는
"piano","bells","square","saw","bass","organ","synth","chime","violin","harp","drum"
이 있습니다.

반복의 경우 하나의 파일에서 첫 번째 이후 인식한 문자열은 모두 무시하고,
피치 조절의 경우 버전 0.5 단위로 조절했던 버전 3과 달리 1 단위로 조절합니다. 

![5](https://github.com/user-attachments/assets/d00f9acd-785d-495d-ba22-3ac1ff37c45a)

버전 3이라면 이후에는 해당 트랙의 악기를 드럼으로 고정할지 묻습니다.
